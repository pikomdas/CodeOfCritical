...
public class Runtime implements UnreportedStepExecutor
{
	...
	// TODO: These are really state machine variables, and I'm not sure the runtime
	// is the best place for this state machine
	...
	public Runtime(ResourceLoader resourceLoader, ClassFinder classFinder, ClassLoader classLoader,
			...
			RuntimeOptions runtimeOptions, StopWatch stopWatch, RuntimeGlue optionalGlue)
	{
		...
		this.glue = optionalGlue != null ? optionalGlue
	...
	private static Collection<? extends Backend> loadBackends(ResourceLoader resourceLoader, ClassFinder classFinder) {
		...
		return reflections.instantiateSubclasses(Backend.class, "cucumber.runtime",
	...
	public void run() throws IOException {
		...
		// TODO: This is duplicated in cucumber.api.android.CucumberInstrumentationCore
		// - refactor or keep uptodate
	...
	public void buildBackendWorlds(Reporter reporter, Set<Tag> tags, Scenario gherkinScenario) {
		...
		// TODO: this is the initial state of the state machine, it should not go here,
	...
	private void runHooks(List<HookDefinition> hooks) throws Throwable {
		if (!runtimeOptions.isDryRun())
		{
			for (HookDefinition hook : hooks)
			{
	...
	private void runHook(HookDefinition hook) throws Throwable {
	...
	// TODO: Maybe this should go into the cucumber step execution model and it
	// should return the result of that execution!
	@Override
	...
	public void runStep(String featurePath, Step step, Reporter reporter, I18n i18n) {
		...
		else
		{
			...
			try
			{
				executeStep(i18n, match, reporter);
			}
			catch (Throwable t)
			{
				error = t;
	...
	private void executeStep(I18n i18n, StepDefinitionMatch match, Reporter reporter) throws Throwable {
		runBeforeStepHooks();
		match.runStep(i18n);
		runAfterStepHooks();
	}
	...
	public void runBeforeStepHooks() throws Throwable {
		runHooks(glue.getBeforeStepsHooks());
	}
