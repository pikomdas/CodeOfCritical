...
public class JavaBackend implements Backend
{
	...
	public JavaBackend(ResourceLoader resourceLoader)
	{
		...
		objectFactory = ObjectFactoryLoader.loadObjectFactory(classFinder,
	...
	public void loadGlue(Glue glue, List<String> gluePaths) {
		...
		{
			Collection<Class<? extends GlueBase>> glueDefinerClasses = classFinder.getDescendants(GlueBase.class,
					packageName(gluePath));
	...
	/**
	 * Convenience method for frameworks that wish to load glue from methods
	 * explicitly (possibly found with a different mechanism than Cucumber's
	 * built-in classpath scanning).
	 *
	 * @param glue where stepdefs and hooks will be added.
	 * @param method a candidate method.
	 * @param glueCodeClass the class implementing the method. Must not be a
	 *        subclass of the class implementing the method.
	 */
	...
	void addStepDefinition(Annotation annotation, Method method) {
		try
		{
			...
			glue.addStepDefinition(
					new JavaStepDefinition(method, pattern(annotation), timeoutMillis(annotation), objectFactory));
		}
	...
	public void addStepDefinition(String regexp, long timeoutMillis, StepdefBody body,
		...
		{
			glue.addStepDefinition(
					new Java8StepDefinition(Pattern.compile(regexp), timeoutMillis, body, typeIntrospector));
		}
		catch (Exception e)
		{
			throw new CucumberException(e);
	...
	void addHook(Annotation annotation, Method method) {
		...
		if (annotation.annotationType().equals(Before.class))
		{
			...
			glue.addBeforeHook(new JavaHookDefinition(method, tagExpressions, ((Before) annotation).order(), timeout,
					objectFactory));
		}
		else if (annotation.annotationType().equals(After.class))
		{
			...
			glue.addAfterHook(new JavaHookDefinition(method, tagExpressions, ((After) annotation).order(), timeout,
					objectFactory));
		}
		else if (annotation.annotationType().equals(BeforeStep.class))
		{
			...
			glue.addBeforeStepHook(new JavaHookDefinition(method, tagExpressions, ((BeforeStep) annotation).order(),
		...
		else if (annotation.annotationType().equals(AfterStep.class))
		{
			...
			glue.addAfterStepHook(new JavaHookDefinition(method, tagExpressions, ((AfterStep) annotation).order(),
					timeout, objectFactory));
		}
	}
