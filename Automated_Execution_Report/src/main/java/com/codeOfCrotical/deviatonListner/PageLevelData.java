 ...
 *   PageLevelData.java belongs to codeOfCrotical
...
import com.codeOfCrotical.CucumberExtentReport.Reporter;
...
public class PageLevelData
{
	 ...
	 * @param attribute
	 */
	public PageLevelData(String scenarioName,
	                     String PageName, String ExpectedValue, String ActualValue, String attribute)
	{
	 ...
	 * @author Partha.Das
	 */
	public PageLevelData(Map<String, Map<String, Double>> mapData)
	{
		mapData.keySet()
				.stream()
				.forEach(pageName ->
				{
					String page = pageName;
					List<String> headers = mapData.get(page)
							.keySet()// Headers
							.stream()
							.filter(headerName -> !(headerName.equalsIgnoreCase("Cost Basis")))
							.filter(headerName -> !(headerName.equalsIgnoreCase("Difference")))
							.collect(Collectors.toList());
//					log.info("=================="+ headers);
					double expectedValue1 = mapData.get(page).get(headers.get(0));
					double expectedValue2 = mapData.get(page).get(headers.get(1));
					log.info(headers.get(0) + " = " + expectedValue1 + " & " + headers.get(1) + " = " + expectedValue2);
					{
						if ((expectedValue1 == 0) && expectedValue2 == 0)
						{
							// Adding Without partnership name with expected Value
							String expectedValueAsString = "Should not be Zero";
							// Adding current partnership name with actual Value
							String actualValueAsString = page + " has " + headers.get(0) + " Value ".concat(String.format("%.8f", expectedValue1) + " (")
									.concat(headers.get(1).concat("=").concat(String.format("%.8f", expectedValue2)).concat(")"));
							log.warn("page: " + page + " Column Name: " + headers.get(0) + " Expected: "
									+ expectedValueAsString + " Actual: " + actualValueAsString);
							// Adding step log to cucumber report
							Reporter.addStepLog("page: " + page + " Column Name: " + headers.get(0) + " Expected: "
									+ expectedValueAsString + " Actual: " + actualValueAsString);
							// Capturing deviation details in a from of GetDetailsOfDeviations object
							getDetailsOfDeviations.set(new GetDetailsOfDeviations(page, headers.get(0),
									expectedValueAsString, actualValueAsString));
							log.info(getDetailsOfDeviations.get());
							// Storing the GetDetailsOfDeviations object into an ArrayList
							data.add(getDetailsOfDeviations.get());
