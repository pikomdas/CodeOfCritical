...
//					log.info("=================="+ headers);
                    // Getting each rows header and values
                    headers.stream()
                            .forEach(header -> {
                                Double actualValue = mapData.get(page).get(header);
                                if ((actualValue != 0) || actualValue.isNaN())
                                {
                                    String expectedValueAsString = "But should not be Zero";
                                    String message = page + " has " + header + " Value ".concat(String.format("%.8f", actualValue)).concat(expectedValueAsString);
                                    Reporter.addStepLog(message);
                                    // Capturing deviation details in a from of GetDetailsOfDeviations object
                                    getDetailsOfDeviations.set(new GetDetailsOfDeviations(page, header, expectedValueAsString, String.format("%.8f", actualValue)));
                                    log.info(getDetailsOfDeviations.get());
                                    // Storing the GetDetailsOfDeviations object into an ArrayList
                                    data.add(getDetailsOfDeviations.get());
                                }
