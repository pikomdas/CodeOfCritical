 ...
 * ExcelFileReadingAndValidation.java belongs to AssetVantage 
 * Do not COPY or PASTE code to WEB from TemenosT24
 * Creation date-time : 24-Apr-2019 10:59:18 AM
 * /////////////////// \\\\\\\\\\\\\\\\\\
 */
 ...
 *
 * <p>
 * This utility will be used to compare Excel File Contents cell by cell for all
 ...
 *
 * <p>
 * (Some of the above attribute comparison only work for *.xlsx format
 * currently. In future it can be enhanced.)
 * </p>
 *
 * <p>
 * <b>Usage:</b>
 ...
 * </pre>
 */
public class ExcelFileReadingAndValidation
{

	@SuppressWarnings("unused")

	private static Logger log = LogManager.getLogger(ExcelFileReadingAndValidation.class.getName());
	...
	public void verify2Excels(String fileLocation1, String fileLocation2) throws Exception {
		// if (args.length != 2 || !(new File(args[0]).exists()) || !(new
		// File(args[1]).exists()))
		// {
		// System.err.println("java -cp <classpath> " +
		// ExcelFileReadingAndValidation.class.getCanonicalName()
		// + " <workbook1.xls/x> <workbook2.xls/x");
		// System.exit(-1);
		// }
		Workbook wb1 = new XSSFWorkbook(new File(fileLocation1));
		Workbook wb2 = new XSSFWorkbook(new File(fileLocation2));

		for (String d : ExcelFileReadingAndValidation.compare(wb1, wb2))
		{
	...
	public static List<String> compare(Workbook wb1, Workbook wb2) {
		...
		ExcelFileReadingAndValidation excelComparator = new ExcelFileReadingAndValidation();
	...
	private void compareNumberOfSheets(Locator loc1, Locator loc2) {
		...
		/*
		 * else { log.info("Number of Sheets do match"); }
		 */ }
	...
	private void compareSheetNames(Locator loc1, Locator loc2) {
		for (int i = 0; i < loc1.workbook.getNumberOfSheets(); i++)
		{
			...
			/*
			 * else { log.info("Name of Sheets do match"); }
			 */
		}
	}
	...
	private void addMessage(Locator loc1, Locator loc2, String messageStart, String value1, String value2) {
		String str = String.format(Locale.ROOT, "%s\nworkbook1 -> %s -> %s [%s] != workbook2 -> %s -> %s [%s]",
				messageStart, loc1.sheet.getSheetName(), new CellReference(loc1.cell).formatAsString(), value1,
				loc2.sheet.getSheetName(), new CellReference(loc2.cell).formatAsString(), value2);
		listOfDifferences.add(str);
		// Adding to Cucumber Report
		Reporter.addStepLog(str);
	}
	...
	private void isCellAlignmentMatches(Locator loc1, Locator loc2) {
		...
		if (align1 != align2)
		{
			addMessage(loc1, loc2, " does not Match ::", align1.name(), align2.name());
		}
		/*
		 * else { log.info("Cell Alignment of Sheets do match"); }
		 */
	}
