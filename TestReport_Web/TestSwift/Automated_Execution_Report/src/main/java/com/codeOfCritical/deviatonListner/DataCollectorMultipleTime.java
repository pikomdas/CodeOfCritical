 ...
 *   DataCollectorMultipleTime.java belongs to codeOfCritical
...
public class DataCollectorMultipleTime {
     ...
     */
    public DataCollectorMultipleTime(String scenarioId,String sessionId, String currentTagName, PageLevelData pageLevelData) {
        this.scenarioId.set(scenarioId);
        this.sessionId.set(sessionId);
        this.currentTagName.set(currentTagName);
        this.pageLevelData.set(pageLevelData);
    }
    ...
    private synchronized JSONObject buildJSON(List<String> screenshots) throws IOException {
        getListonDeviations()
                ...
                .forEach(x ->
                {
                    jsonBuilder.get().collectData(scenarioId.get(),sessionId.get(), currentTagName.get(), x.getScenarioName(), x.getPageName(), x.getFieldName(), x.getExpectedValue(),
